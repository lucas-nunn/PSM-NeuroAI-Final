...
class DecBlock(nn.Module):
    ...
    def sample(self, x, acts, deterministic=False):
        ...
        x = x + xpp
        # PATCHED: `deterministic` takes the posterior mean instead of sampling.
        # This block's qm is computed from the decoder state x, which shallower
        # blocks built up via `x = x + z_fn(z)` -- so sampling anywhere upstream
        # makes every deeper qm stochastic too. Propagating the mean is what makes
        # the whole hierarchy reproducible, not just this block.
        z = qm if deterministic else draw_gaussian_diag_samples(qm, qv)
        kl = gaussian_analytical_kl(qm, pm, qv, pv)
        return z, x, kl, qm, qv          # PATCHED: also return the posterior q(z|x)
    ...
    def forward(self, xs, activations, get_latents=False, deterministic=False):
        x, acts = self.get_inputs(xs, activations)
        if self.mixin is not None:
            x = x + F.interpolate(xs[self.mixin][:, :x.shape[1], ...], scale_factor=self.base // self.mixin)
        z, x, kl, qm, qv = self.sample(x, acts, deterministic=deterministic)
        x = x + self.z_fn(z)
        x = self.resnet(x)
        xs[self.base] = x
        if get_latents:
            # PATCHED: upstream returned only the *sample* z. z is drawn from
            # q(z|x), so an embedding built from it is stochastic -- the same image
            # would yield a different RDM on every call. qm (the posterior mean) is
            # the deterministic summary we actually want for RSA/encoding.
            return xs, dict(z=z.detach(), qm=qm.detach(), qv=qv.detach(), kl=kl)
...
class Decoder(HModule):
    ...
    def forward(self, activations, get_latents=False, deterministic=False):
        stats = []
...
class VAE(HModule):
    ...
    def forward_get_latents(self, x, deterministic=False):
        activations = self.encoder.forward(x)
